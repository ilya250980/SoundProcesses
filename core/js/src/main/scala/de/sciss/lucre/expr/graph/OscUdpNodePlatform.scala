...
import de.sciss.osc
import de.sciss.osc.{Browser, Packet}
...
// XXX TODO DRY with JVM
trait OscUdpNodePlatform {
  type PeerSocketAddress = Browser.Address
  ...
  protected final class SenderExpanded[T <: Txn[T]](peer: Repr[T], tx0: T)
    ...
    def value(implicit tx: T): SocketAddress = SocketAddress("localhost", peer.sender.port)
    ...
    private[lucre] def pullChange(pull: IPull[T])(implicit tx: T, phase: IPull.Phase): SocketAddress = {
      ...
      SocketAddress("localhost", addr.port)
    }
  ...
  protected final class Expanded[T <: Txn[T]](protected val peer: OscUdpNode, localPort: Int, localHost: String)
    ...
    private[this] val dummySocket = Browser.Address(0)
    ...
    private[this] val receiverFun: (Packet, Browser.Address) => Unit = { (p, addr) =>
    ...
    @volatile
    private[this] var transmitter : osc.Browser.Transmitter .Undirected = _
    private[this] var receiver    : osc.Browser.Receiver    .Undirected = _
    ...
    private def sendNow(target: SocketAddress, p: OscPacket): Unit = {
      ...
      if (lastTarget == target) sendWith(lastSck, p)
        ...
        val res = Browser.Address(target.port)
...
//          }
    ...
    override def initControl()(implicit tx: T): Unit = {
      ...
      tx.afterCommit {
        val cfgB = osc.Browser.Config()
        cfgB.codec = codec
        cfgB.localAddress = Browser.Address(localPort)
        val cfg         = cfgB.build
        tryThunk("initialize") {
          val t       = osc.Browser.Transmitter(cfg)
          t.connect()
          transmitter = t
          val r       = osc.Browser.Receiver(/*transmitter.channel,*/ cfg)
